...
| thurbox discovery | Conditional | thurbox `2.19.0` or newer is a second, optional global-only integration beside Herdr. Proqi verifies the release, resolves the registered agent holding each session, and projects an honest `idle`/`working`/`done`/`blocked`/`running`/`unreported`/`uncovered` state without pane adjacency, a direction chooser, or a `Live in thurbox` invocation-picker group. `PROQI_DISABLE_THURBOX=1` removes it. | `src/adapters/thurbox`, `tests/thurbox_executable.rs` |
...
| Direct process execution | Shipped | Herdr, thurbox, installer, and verification commands use argument vectors and optional standard input without shell interpolation. | `src/adapters/process/mod.rs`, process tests |
